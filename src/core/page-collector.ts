...
/**
 * Page data collectors for rc-devtools-mcp.
 * Collects network requests, console messages, and issues from browser pages.
 */

import {FakeIssuesManager} from '../utils/devtools-utils.js';
...
export class PageCollector<T> {
  ...
  dispose() {
    for (const browser of this.#browsers) {
      ...
      browser.off('targetdestroyed', this.#onTargetDestroyed);
    }
  }
...
export class NetworkCollector extends PageCollector<HTTPRequest> {
  ...
  override addPage(page: Page): void {
    super.addPage(page);
    enableNetworkInitiatorTracking(page).catch(err => {
      logger('Error enabling network initiator tracking', err);
    });
  }

  override splitAfterNavigation(page: Page) {
    clearNetworkInitiators(page);
    ...
    if (lastRequestIdx !== -1) {
      let earliestIdx = lastRequestIdx;
      const lastRequest = requests[lastRequestIdx];
      const redirectChain = lastRequest.redirectChain();
      if (redirectChain.length > 0) {
        const firstRedirectRequest = redirectChain[0];
        const firstRedirectIdx = requests.findIndex(
          req => req === firstRedirectRequest,
        );
        if (firstRedirectIdx !== -1 && firstRedirectIdx < earliestIdx) {
          earliestIdx = firstRedirectIdx;
        }
      }

      const fromCurrentNavigation = requests.splice(earliestIdx);
